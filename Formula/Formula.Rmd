...
lindata = lapply(scales, function(scale) { 
    read_delim(paste0('resLines', scale, '.txt'), delim = ';', locale = locale(decimal_mark = ".")) %>% 
    select(-ncol(.)) %>% 
    mutate(Scale = scale,
...
pntdata = lapply(scales, function(X) { 
    read_delim(paste0('resPoints', X, '.txt'), delim = ';', locale = locale(decimal_mark = ".")) %>% 
    select(-ncol(.)) %>% 
...
poldata = lapply(scales, function(X) { 
    read_delim(paste0('resPolygons', X, '.txt'), delim = ';', locale = locale(decimal_mark = ".")) %>% 
    select(-ncol(.)) %>% 
    mutate(Scale = as.character(X),
           Region = stringr::str_sub(Region, 1, nchar(Region) - nchar(Scale)))
  }) %>% 
...
inters = read_delim('topLines.txt', delim = ';', locale = locale(decimal_mark = ".")) %>% 
  select(-ncol(.)) %>% 
