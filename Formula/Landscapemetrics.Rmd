...
```{r denstats, fig.width=10, fig.height=14, fig.cap='Относительные параметры геометрической детализации карты (плотность)'}
...
```{r densbox, fig.width=10, fig.height=5, fig.cap='Разброс относительных параметров геометрической детализации карты (плотность)'}
...
```{r sizestats, fig.width=10, fig.height=14, fig.cap='Абсолютные параметры геометрической детализации карты (размеры)'}
...
```{r sizebox, fig.width=10, fig.height=5, fig.cap='Разброс абсолютных параметров геометрической детализации карты (плотность)'}
...
где $d_i$ — величина $i$-й нормированной на среднее по масштабу метрики детализации, $u$ — доля урбанизированных территорий, $с$ — доля сельскохозяйственных угодий. Результаты регрессионного анализа приведены в Таблице \@ref(tab:coef). Коэффициенты, полученны по данным со всех масштабов, помечены как `NA`.
...
На заключительном этапе анализа была проанализирована возможность нормировать плотность объектов на уровень хозяйственной освоенности с использованием найденных регрессионных уравнений.

```{r denstatsnorm, fig.width=10, fig.height=14, fig.cap='Относительные параметры геометрической детализации карты (плотность)'}
...
```{r densboxnorm, fig.width=10, fig.height=5, fig.cap='Разброс относительных параметров геометрической детализации карты (плотность)'}
